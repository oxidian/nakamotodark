...
int nCoinbaseMaturity = 60; // 60 blocks for coins to mature
...
const string strMessageMagic = "nakamoto Signed Message:\n";
...
bool CTransaction::CheckTransaction() const
{
    ...
    BOOST_FOREACH(const CTxIn& txin, vin)
    {
        printf("\nTransaction IN Marker: %s", txin.ToString().c_str());
        printf("Transaction in Marker SHORT: %s\n", txin.ToStringShort().c_str());
...
static const int64_t nTargetTimespan = 16 * 30;  // 8 mins
//
...
// nakamoto: attempt to generate suitable proof-of-stake
...
bool CheckDiskSpace(uint64_t nAdditionalBytes)
{
    ...
    if (nFreeBytesAvailable < nMinDiskSpace + nAdditionalBytes)
    {
        ...
        printf("*** %s\n", strMessage.c_str());
        uiInterface.ThreadSafeMessageBox(strMessage, "nakamoto", CClientUIInterface::OK | CClientUIInterface::ICON_EXCLAMATION | CClientUIInterface::MODAL);
        StartShutdown();
        return false;
    }
    return true;
...
bool LoadBlockIndex(bool fAllowNew)
{
    ...
    if (mapBlockIndex.empty())
    {
        ...
        const char* pszTimestamp = "Nakamotocoin tribute";
        CTransaction txNew;
        txNew.nTime = 1418833531;
        txNew.vin.resize(1);
        txNew.vout.resize(1);
        ...
        //// debug print
        assert(block.hashMerkleRoot == uint256("0x379f0962c5aa259666bfff275b0dfe8415399ef09847a201a9c28be167047038"));
        block.print();
...
unsigned char pchMessageStart[4] = { 0xf3, 0x2d, 0xa5, 0x71 };
